...
class Author(db.Model):
    ...
    def validate_phone_number(self, key, phone_number):
        if (len(phone_number) != 10) or (not phone_number.isdigit()):
            raise ValueError("Phone number need to be exactly ten digits")
        return phone_number
...
class Post(db.Model):
    ...
    def validate_content(self, key, content):
        if len(content) < 250:
            raise ValueError("Post content need to be at least 250 characters long.")
        return content
    ...
    def validate_title(self, key, title):
        ...
        for keyword in clickbait_keywords:
            if keyword in title:
                return title
        raise ValueError("Title must contains the clickbait keywords.")
